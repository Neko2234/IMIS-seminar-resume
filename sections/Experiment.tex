...
プレース位置の変位 $\bm{c} = [\mathit{c}_x,\, \mathit{c}_y]$（各軸方向への移動量，単位 cm）をラベルとして与えたとき，
...
次に Motion ReTouch を用いて，プレース位置を $X$・$Y$ 方向にそれぞれ $\pm5\,\text{cm}$ ずらした 8 箇所と，何も修正しなかった場合の 1 箇所，計 9 箇所に対して各 5 回の修正動作を収録し，合計 45 本のデータを収集した．
各修正動作にラベル $\bm{c} = [\mathit{c}_x, \mathit{c}_y]$ を付与し，
ベース動作データとの差分 $\Delta\bm{\xi}^{(\bm{c})}$ を教師残差とした．
...
\begin{table*}[t]
  ...
  \begin{tabular}{llll}
    ...
    \texttt{Residual w/ image} & 関節状態・画像・ラベル & 残差 $\Delta\bm{\xi}$ & ベース再生＋残差加算 \\
    \texttt{Residual w/o image} & 関節状態・ラベル & 残差 $\Delta\bm{\xi}$ & 画像入力なし \\
    \hline
  \end{tabular}
\end{table*}
...
\texttt{Residual w/ image} と \texttt{Residual w/o image} は残差 $\Delta\bm{\xi}$ を予測し，
...
評価指標として以下を用いた：
\begin{itemize}
  \item ラベルと実変位の決定係数 $R^2$ および Spearman 順位相関係数 $\rho$（線形性の指標）
  \item ラベル $\bm{c} = [0,0]$ 時のプレース位置オフセット（残差ゼロの再現精度）
  \item 同一ラベル内の試行間標準偏差 $\sigma$（再現性）
  \item ピック時の手先位置偏差（ピック動作への残差干渉の指標）
\end{itemize}
