...
\begin{figure}[t]
  ...
  \caption{$\bm{c}=[0,0]$ 時のプレース位置分布（ラベル座標系）．$+$ はモーションコピーの平均位置（基準点 $(0,0)$）を示す．}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    \hline
    モデル & $n$ & オフセット [mm] & $\sigma_{c_x} / \sigma_{c_y}$ [mm] \\
    \hline
...
\texttt{Motion Copy}（$\sigma_{c_x} \approx 0.30$\,mm, $\sigma_{c_y} \approx 0.21$\,mm）はハードウェア再現精度の上限を示す．
...
ピック段階への干渉が相対的に少ないが，その代わりラベル追従性が低い．
